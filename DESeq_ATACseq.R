...
# select code chunk; ctrl + shift + c to comment all lines out
...
# metaData <- data.frame(Group=Group,Group2=prostatevslymph,row.names=colnames(myCounts))
# 
...
# 
# atacDDS_prostatevsblood <- DESeqDataSetFromMatrix(myCounts,metaData,~Group2,rowRanges=consensusToCount)
# atacDDS_prostatevsblood <- DESeq(atacDDS_prostatevsblood)
# atac_Rlog_prostatevsblood <- rlog(atacDDS_prostatevsblood)
# save(atacDDS_prostatevsblood,file=paste0("atacDDS_Park_Fanying_GM12878_prostatevslymphoblastoid.RData"))
# save(atac_Rlog_prostatevsblood,file=paste0("atac_Rlog_Park_Fanying_GM12878_prostatevslymphoblastoid.RData"))
# 
...
# 
# pca_signal <- plotPCA(atac_Rlog_prostatevsblood,intgroup="Group2",ntop=nrow(atac_Rlog_prostatevsblood)) + geom_label_repel(label = rownames(colData(atac_Rlog_prostatevsblood)), nudge_y=50, nudge_x=50)
# savePlot(pca_signal, "pca_signal_prostatevsblood")
# 
# ###
...
# 
# pdf(paste0("differential_accessibility.pdf"))
...
load(paste0("atacDDS_Park_Fanying_GM12878_prostatevslymphoblastoid.RData"))
load(paste0("atac_Rlog_Park_Fanying_GM12878_prostatevslymphoblastoid.RData"))
...
prostateminuslymphoblastoid <- results(atacDDS_prostatevsblood,c("Group2","prostate","lymphoblastoid"),format="GRanges")
prostateminuslymphoblastoid <- prostateminuslymphoblastoid[order(prostateminuslymphoblastoid$pvalue)]
head(prostateminuslymphoblastoid)
tail(prostateminuslymphoblastoid)
...
prostateminuslymphoblastoid <- prostateminuslymphoblastoid[(!is.na(prostateminuslymphoblastoid$padj) & prostateminuslymphoblastoid$padj < 0.05) & prostateminuslymphoblastoid %over% toOverLap,]
#makebedtable(prostateminuslymphoblastoid,"prostateminuslymphoblastoid.html", ".")

CRPCMinusNEPC <- CRPCMinusNEPC[(!is.na(CRPCMinusNEPC$padj) & CRPCMinusNEPC$padj < 0.05) & CRPCMinusNEPC %over% toOverLap,]
#makebedtable(CRPCMinusNEPC,"CRPCMinusNEPC.html", ".")
...
library(ChIPseeker)
library(DT)
anno_prostateminuslymphoblastoid <- annotatePeak(prostateminuslymphoblastoid,TxDb = TxDb.Hsapiens.UCSC.hg19.knownGene)
go1 <- enrichGO(as.data.frame(as.GRanges(anno_prostateminuslymphoblastoid)[as.GRanges(anno_prostateminuslymphoblastoid)$log2FoldChange > 0])$geneId,
                OrgDb = "org.Hs.eg.db",ont = "BP",maxGSSize = 5000)
go2 <- enrichGO(as.data.frame(as.GRanges(anno_prostateminuslymphoblastoid)[as.GRanges(anno_prostateminuslymphoblastoid)$log2FoldChange < 0])$geneId, 
                OrgDb = "org.Hs.eg.db",ont = "BP",maxGSSize = 5000)
...
anno_CRPCMinusNEPC <- annotatePeak(CRPCMinusNEPC,TxDb = TxDb.Hsapiens.UCSC.hg19.knownGene)
go1 <- enrichGO(as.data.frame(as.GRanges(anno_CRPCMinusNEPC)[as.GRanges(anno_CRPCMinusNEPC)$log2FoldChange > 0])$geneId,
                OrgDb = "org.Hs.eg.db",ont = "BP",maxGSSize = 5000)
go2 <- enrichGO(as.data.frame(as.GRanges(anno_CRPCMinusNEPC)[as.GRanges(anno_CRPCMinusNEPC)$log2FoldChange < 0])$geneId, 
                OrgDb = "org.Hs.eg.db",ont = "BP",maxGSSize = 5000)
head(go1,10) %>% dplyr::select(ID,Description,pvalue,p.adjust) %>% datatable(elementId = "goEle1")
head(go2,10) %>% dplyr::select(ID,Description,pvalue,p.adjust) %>% datatable(elementId = "goEle2")
